...
NOTE: Apache commons is broken. It's possible to hack a serialized object and gain root access to the application server. Apache commons is used by most application servers. This is important to know if (want) to use RMI.
...
[source,java,linenums]
----
include::{projectdir}/contacts/src/main/com/airhacks/tight/monitoring/boundary/Monitoring.java[]
----
