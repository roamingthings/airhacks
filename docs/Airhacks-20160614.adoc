...
* For the container part of security look at the upcoming https://github.com/javaee-security-spec/soteria[Java EE Security JSR] specification
...
* BCE
** Boundary package is named _boundary_
*** `package com.company.contacts.boundary;`
...
** `XXXConverter` are put into the concept package (`com.company.date.DateTimeConverter`)
...
NOTE: Apache commons is broken. It's possible to hack a serialized object and gain root acces to the application server. Apache commons is used by most application servers.
